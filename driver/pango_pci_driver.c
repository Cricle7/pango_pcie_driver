...
{
	unsigned long ret =  copy_to_user(buf, &command_operation, sizeof(COMMAND_OPERATION));
	ret=1;
	return ret;
}
...
long pango_cdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		case PCI_READ_DATA_CMD:
			ret=copy_from_user(&config_operation,(COMMAND_OPERATION*)arg, sizeof(COMMAND_OPERATION));
			pci_read_config_dword(op_dev, config_operation.addr, &config_operation.data);
			ret=copy_to_user((COMMAND_OPERATION*)arg, &config_operation, sizeof(COMMAND_OPERATION));
		break;
		...
		case PCI_MAP_ADDR_CMD:
			...
			ret=copy_from_user(&dma_operation,(DMA_OPERATION*)arg, sizeof(DMA_OPERATION));
			dma_info.addr_r.data_buf = dma_alloc_coherent(&op_dev->dev, dma_operation.current_len*4, &dma_info.addr_r.addr,GFP_KERNEL);
			dma_info.addr_w.data_buf = dma_alloc_coherent(&op_dev->dev, dma_operation.current_len*4, &dma_info.addr_w.addr,GFP_KERNEL);
		...
		case PCI_READ_FROM_KERNEL_CMD:										/* 将数据写入CPU缓存 */
			...
			ret=copy_to_user((DMA_OPERATION*)arg, &dma_operation, sizeof(DMA_OPERATION));
			spin_unlock(&dma_info.addr_r.lock);
		break;

		case PCI_UMAP_ADDR_CMD:
			spin_lock(&dma_info.addr_r.lock);
			dma_free_coherent(&op_dev->dev, dma_operation.current_len*4, dma_info.addr_r.data_buf, dma_info.addr_r.addr);
			dma_free_coherent(&op_dev->dev, dma_operation.current_len*4, dma_info.addr_w.data_buf, dma_info.addr_w.addr);
		...
		case PCI_PERFORMANCE_START_CMD:
			...
			ret=copy_from_user(&performance_operation,(PERFORMANCE_OPERATION*)arg, sizeof(PERFORMANCE_OPERATION));
			performance_config.addr.data_buf = dma_alloc_coherent(&op_dev->dev, DMA_MAX_PACKET_SIZE*9*2, &performance_config.addr.addr, GFP_KERNEL);
		...
		case PCI_PERFORMANCE_END_CMD:
			...
			ret=copy_to_user((PERFORMANCE_OPERATION*)arg, &performance_operation, sizeof(PERFORMANCE_OPERATION));
...
int init_pango_cdev_class(struct class *pango_cdev_class)
{
    ...
    pango_cdev_class = class_create(pci_dev_info._dev_name);
#else
