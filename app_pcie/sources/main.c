...
SCREEN_INFO GuiScreen[] = 
{
	...
	/* Close DMA Button */
	{105,  "Close DMA",BUTTON,NOTEBOOK_FRAME_FIXED_INDEX_2,but_w,but_h,ex2,ey2+es2*3+10,	TRUE,	close_dma_button,"clicked",	"0"},
	/* DMA Write Button */
...
gboolean dma_write_button( GtkWidget *Widget, gpointer Data )
{
	...
	{
		printf_info("DMA Write (PCI_DMA_WRITE_CMD)\n");
		#ifdef NO_TEST
...
gboolean close_dma_button( GtkWidget *Widget, gpointer Data )
{
	printf_info("Close DMA Manual Test (PCI_UMAP_ADDR_CMD)\n");
	button_flag.manual_start = BOOL_SWITCH(button_flag.manual_start);
	enable_start_dma_button();
	#ifdef NO_TEST
		ioctl(pci_driver_fd, PCI_UMAP_ADDR_CMD, &dma_operation);						/* 释放数据缓存 */
	#endif
	free_file_buf(dma_write_file_info.file_data_buffer);
}
